...
Canonical MRN medical dataset (v2 retrieval):

```bash
python3 data/generate_dataset.py
```

Legacy privacy benchmark dataset (compatibility):
...
## Retrieval Benchmark (v2 Canonical)

The current retrieval benchmark is **v2** (MRN medical dataset):

- **Ground truth:** `benchmarks/retrieval/ground_truth_v2.json` (605 queries, multi-record aggregates)
- **Retrieval results:** `benchmarks/retrieval/retrieval_results_v2.json` (k=50, dense retrieval)
- **Metrics:** `benchmarks/retrieval/metrics_v2.json` and `failure_analysis_v2.json`
- **Depth report:** `reports/retrieval/dense/depth_report.md`
- **Canonical dataset:** `data/sample_patient_data.txt` generated by `data/generate_dataset.py`

Historical v1 artifacts are archived at `benchmarks/retrieval/_archive_v1/`.
